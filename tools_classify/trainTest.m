% function modelData = getModelWithGnd(conf, classData, options)
% if not (nargin > 2)
%     options = struct;
% end
...
conf.modelFile = '/Users/justin/Documents/MATLAB/medsim/data/emotion/modelFile.mat';
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
classCount = max(unique(signalGnd));
classData.classesAssigned = unique(signalGnd);
for c = 1:classCount
...
for i = 1:length(signalGnd)
  if signalGnd(i) ~= currentClass
    ...
    thisSetSize = length(classData.continuousClassSignals.(classLabels{currentClass}));
    classData.continuousClassSignals.(classLabels{currentClass}){thisSetSize+1} = thisSegment;
    classData.classSampleCounts.(classLabels{currentClass}) = classData.classSampleCounts.(classLabels{currentClass}) + length(thisSegment);
