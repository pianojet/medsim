...
%      Version: $Id: estnoiseg.m 1718 2012-03-31 16:40:41Z dmb $
%
...
if isempty(yf) && isstruct(tz)             % no real data
    ...
    tax=qq.tax;    % noise output smoothing time constant = -tinc/log(0.8) (8)
    tap=qq.tap;    % noise output smoothing time constant = -tinc/log(0.9) (23)
    psthr=qq.psthr;  % threshold for smoothed speech probability [0.99] (24)
    pnsaf=qq.pnsaf;      % noise probability safety value [0.01] (24)
    pspri=qq.pspri;       % prior speech probability [0.5] (18)
    asnr=qq.asnr;         % active SNR in dB [15] (18)
    psini=qq.psini;       % initial speech probability [0.5] (23)
    tavini=qq.tavini;     % assumed speech absent time at start [0 seconds]

    % derived algorithm constants

    ax=exp(-tinc/tax); % noise output smoothing factor = 0.8 (8)
    axc=1-ax;
    ap=exp(-tinc/tap); % noise output smoothing factor = 0.9 (23)
    apc=1-ap;
    xih1=10^(asnr/10); % speech-present SNR
    xih1r=1/(1+xih1)-1;
    pfac=(1/pspri-1)*(1+xih1); % p(noise)/p(speech) (18)

    if ~nrcum       % initialize values for first frame
        xt=psini*mean(yf(1:max(1,min(nr,round(1+tavini/tinc))),:),1);  % initial noise estimate
        pslp=repmat(psini,1,nrf); % initialize smoothed speech presence prob
    end
